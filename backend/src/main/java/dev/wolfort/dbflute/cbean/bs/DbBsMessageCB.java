 ...
 */
public class DbBsMessageCB extends AbstractConditionBean {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected DbMessageCQ _conditionQuery;

    // ===================================================================================
    //                                                                         Constructor
    //                                                                         ===========
    public DbBsMessageCB() {
     ...
     * @return this. (NotNull)
     */
    public DbMessageCB acceptPK(Long messageId, Integer gameId) {
        assertObjectNotNull("messageId", messageId);assertObjectNotNull("gameId", gameId);
        DbBsMessageCB cb = this;
        cb.query().setMessageId_Equal(messageId);cb.query().setGameId_Equal(gameId);
        return (DbMessageCB)this;
    }
     ...
     * @return this. (NotNull)
     */
    public DbMessageCB acceptUniqueOf(Integer gameId, String messageTypeCode, Integer messageNumber) {
        ...
        DbBsMessageCB cb = this;
        cb.query().setGameId_Equal(gameId);cb.query().setMessageTypeCode_Equal(messageTypeCode);cb.query().setMessageNumber_Equal(messageNumber);
        return (DbMessageCB)this;
    }
     ...
     */
    public DbMessageCQ query() {
        assertQueryPurpose(); // assert only when user-public query
        return doGetConditionQuery();
    }

    public DbMessageCQ xdfgetConditionQuery() { // public for parameter comment and internal
        return doGetConditionQuery();
    }

    protected DbMessageCQ doGetConditionQuery() {
    ...
    protected DbMessageCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause().getBasePointAliasName(), 0);
    }

    protected DbMessageCQ xcreateCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        DbMessageCQ cq = xnewCQ(childQuery, sqlClause, aliasName, nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected DbMessageCQ xnewCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        return new DbMessageCQ(childQuery, sqlClause, aliasName, nestLevel);
    }
     ...
     */
    public void union(UnionQuery<DbMessageCB> unionCBLambda) {
        final DbMessageCB cb = new DbMessageCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final DbMessageCQ cq = cb.query(); query().xsetUnionQuery(cq);
    }
     ...
     */
    public void unionAll(UnionQuery<DbMessageCB> unionCBLambda) {
        final DbMessageCB cb = new DbMessageCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final DbMessageCQ cq = cb.query(); query().xsetUnionAllQuery(cq);
    }
     ...
     *     ... = <span style="color: #553000">message</span>.<span style="color: #CC4747">getGame()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
     */
     ...
     *     <span style="color: #553000">cb</span>.query().set...
     * }).alwaysPresent(<span style="color: #553000">message</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     ... = <span style="color: #553000">message</span>.<span style="color: #CC4747">getGamePeriod()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
     ...
     *     <span style="color: #553000">cb</span>.query().set...
     * }).alwaysPresent(<span style="color: #553000">message</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     ... = <span style="color: #553000">message</span>.<span style="color: #CC4747">getGamePlayer()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
     * @return The set-upper of nested relation. {setupSelect...().with[nested-relation]} (NotNull)
     */
    ...
    public static class HpSpecification extends HpAbstractSpecification<DbMessageCQ> {
        ...
        public HpSpecification(ConditionBean baseCB, HpSpQyCall<DbMessageCQ> qyCall
         ...
         * @return The information object of specified column. (NotNull)
         */
        public SpecifiedColumn columnRepliedCount() { return doColumn("replied_count"); }
        /**
         * favored_count: {NotNull, INT UNSIGNED(10)}
         * @return The information object of specified column. (NotNull)
         */
        public SpecifiedColumn columnFavoredCount() { return doColumn("favored_count"); }
        /**
         ...
         */
        public HpSDRFunction<DbMessageReplyCB, DbMessageCQ> derivedMessageReplyByFromMessageId() {
            assertDerived("messageReplyByFromMessageIdList"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<DbMessageReplyCB> sq, DbMessageCQ cq, String al, DerivedReferrerOption op)
                    -> cq.xsderiveMessageReplyByFromMessageIdList(fn, sq, al, op), _dbmetaProvider);
        }
        /**
         * Prepare for (Specify)DerivedReferrer (correlated sub-query). <br>
         * {select max(FOO) from message_reply where ...) as FOO_MAX} <br>
         ...
         * @return The object to set up a function for referrer table. (NotNull)
         */
        public HpSDRFunction<DbMessageReplyCB, DbMessageCQ> derivedMessageReplyByReplyMessageId() {
            assertDerived("messageReplyByReplyMessageIdList"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<DbMessageReplyCB> sq, DbMessageCQ cq, String al, DerivedReferrerOption op)
                    -> cq.xsderiveMessageReplyByReplyMessageIdList(fn, sq, al, op), _dbmetaProvider);
        }
        /**
         * Prepare for (Specify)DerivedReferrer (correlated sub-query). <br>
         ...
         * @return The object to set up a function for referrer table. (NotNull)
         */
        public HpSDRFunction<DbMessageFavoriteCB, DbMessageCQ> derivedMessageFavorite() {
            assertDerived("messageFavoriteList"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<DbMessageFavoriteCB> sq, DbMessageCQ cq, String al, DerivedReferrerOption op)
     ...
     */
    public DbMessageCB dreamCruiseCB() {
        DbMessageCB cb = new DbMessageCB();
        cb.xsetupForDreamCruise((DbMessageCB) this);
        return cb;
    }
     ...
     */
    public HpColQyOperand<DbMessageCB> columnQuery(final SpecifyQuery<DbMessageCB> colCBLambda) {
        return xcreateColQyOperand((rightSp, operand) -> {
            return xcolqy(xcreateColumnQueryCB(), xcreateColumnQueryCB(), colCBLambda, rightSp, operand);
        });
    }

    protected DbMessageCB xcreateColumnQueryCB() {
        DbMessageCB cb = new DbMessageCB();
        cb.xsetupForColumnQuery((DbMessageCB)this);
        return cb;
    }
     ...
     */
    public void orScopeQuery(OrQuery<DbMessageCB> orCBLambda) {
        xorSQ((DbMessageCB)this, orCBLambda);
    }
     ...
     */
    public void orScopeQueryAndPart(AndQuery<DbMessageCB> andCBLambda) {
        xorSQAP((DbMessageCB)this, andCBLambda);
    }
    ...
    protected void xprepareSyncQyCall(ConditionBean mainCB) {
        final DbMessageCB cb;
        if (mainCB != null) {
            cb = (DbMessageCB)mainCB;
        } else {
            cb = new DbMessageCB();
        }
    ...
    protected String xgetConditionBeanClassNameInternally() { return DbMessageCB.class.getName(); }
    protected String xgetConditionQueryClassNameInternally() { return DbMessageCQ.class.getName(); }
