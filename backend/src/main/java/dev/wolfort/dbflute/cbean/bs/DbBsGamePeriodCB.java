...
/**
 * The base condition-bean of game_period.
 * @author DBFlute(AutoGenerator)
 */
public class DbBsGamePeriodCB extends AbstractConditionBean {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected DbGamePeriodCQ _conditionQuery;

    // ===================================================================================
    //                                                                         Constructor
    //                                                                         ===========
    public DbBsGamePeriodCB() {
     ...
     * @return this. (NotNull)
     */
    public DbGamePeriodCB acceptPK(Integer gamePeriodId) {
        assertObjectNotNull("gamePeriodId", gamePeriodId);
        DbBsGamePeriodCB cb = this;
        cb.query().setGamePeriodId_Equal(gamePeriodId);
        return (DbGamePeriodCB)this;
    }
     ...
     * @return this. (NotNull)
     */
    public DbGamePeriodCB acceptUniqueOf(Integer gameId, Integer period) {
        assertObjectNotNull("gameId", gameId);assertObjectNotNull("period", period);
        DbBsGamePeriodCB cb = this;
        cb.query().setGameId_Equal(gameId);cb.query().setPeriod_Equal(period);
        return (DbGamePeriodCB)this;
    }

    public ConditionBean addOrderBy_PK_Asc() {
        query().addOrderBy_GamePeriodId_Asc();
        return this;
    }

    public ConditionBean addOrderBy_PK_Desc() {
        query().addOrderBy_GamePeriodId_Desc();
        return this;
    }
     ...
     */
    public DbGamePeriodCQ query() {
        assertQueryPurpose(); // assert only when user-public query
        return doGetConditionQuery();
    }

    public DbGamePeriodCQ xdfgetConditionQuery() { // public for parameter comment and internal
        return doGetConditionQuery();
    }

    protected DbGamePeriodCQ doGetConditionQuery() {
    ...
    protected DbGamePeriodCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause().getBasePointAliasName(), 0);
    }

    protected DbGamePeriodCQ xcreateCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        DbGamePeriodCQ cq = xnewCQ(childQuery, sqlClause, aliasName, nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected DbGamePeriodCQ xnewCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        return new DbGamePeriodCQ(childQuery, sqlClause, aliasName, nestLevel);
    }
     ...
     */
    public void union(UnionQuery<DbGamePeriodCB> unionCBLambda) {
        final DbGamePeriodCB cb = new DbGamePeriodCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final DbGamePeriodCQ cq = cb.query(); query().xsetUnionQuery(cq);
    }
     ...
     */
    public void unionAll(UnionQuery<DbGamePeriodCB> unionCBLambda) {
        final DbGamePeriodCB cb = new DbGamePeriodCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final DbGamePeriodCQ cq = cb.query(); query().xsetUnionAllQuery(cq);
    }
     ...
     * <pre>
     * <span style="color: #0000C0">gamePeriodBhv</span>.selectEntity(<span style="color: #553000">cb</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     ...
     * }).alwaysPresent(<span style="color: #553000">gamePeriod</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     ... = <span style="color: #553000">gamePeriod</span>.<span style="color: #CC4747">getGame()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
     */
    ...
    public static class HpSpecification extends HpAbstractSpecification<DbGamePeriodCQ> {
        protected DbGameCB.HpSpecification _game;
        public HpSpecification(ConditionBean baseCB, HpSpQyCall<DbGamePeriodCQ> qyCall
         ...
         * @return The information object of specified column. (NotNull)
         */
        public SpecifiedColumn columnPeriod() { return doColumn("period"); }
        /**
         * prefix: {VARCHAR(255)}
         * @return The information object of specified column. (NotNull)
         */
        public SpecifiedColumn columnPrefix() { return doColumn("prefix"); }
        /**
         * suffix: {VARCHAR(255)}
         * @return The information object of specified column. (NotNull)
         */
        public SpecifiedColumn columnSuffix() { return doColumn("suffix"); }
        /**
        ...
        @Override
        protected String getTableDbName() { return "game_period"; }
        /**
         ...
         */
        public HpSDRFunction<DbMessageCB, DbGamePeriodCQ> derivedMessage() {
            assertDerived("messageList"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<DbMessageCB> sq, DbGamePeriodCQ cq, String al, DerivedReferrerOption op)
         ...
         * @return The object to set up a function for referrer table. (NotNull)
         */
        public HpSDRFunction<DbDirectMessageCB, DbGamePeriodCQ> derivedDirectMessage() {
            assertDerived("directMessageList"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<DbDirectMessageCB> sq, DbGamePeriodCQ cq, String al, DerivedReferrerOption op)
         ...
         */
        public HpSDRFunction<DbGamePeriodCB, DbGamePeriodCQ> myselfDerived() {
            assertDerived("myselfDerived"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<DbGamePeriodCB> sq, DbGamePeriodCQ cq, String al, DerivedReferrerOption op)
     ...
     */
    public DbGamePeriodCB dreamCruiseCB() {
        DbGamePeriodCB cb = new DbGamePeriodCB();
        cb.xsetupForDreamCruise((DbGamePeriodCB) this);
        return cb;
    }
     ...
     */
    public HpColQyOperand<DbGamePeriodCB> columnQuery(final SpecifyQuery<DbGamePeriodCB> colCBLambda) {
        return xcreateColQyOperand((rightSp, operand) -> {
            return xcolqy(xcreateColumnQueryCB(), xcreateColumnQueryCB(), colCBLambda, rightSp, operand);
        });
    }

    protected DbGamePeriodCB xcreateColumnQueryCB() {
        DbGamePeriodCB cb = new DbGamePeriodCB();
        cb.xsetupForColumnQuery((DbGamePeriodCB)this);
        return cb;
    }
     ...
     */
    public void orScopeQuery(OrQuery<DbGamePeriodCB> orCBLambda) {
        xorSQ((DbGamePeriodCB)this, orCBLambda);
    }
     ...
     */
    public void orScopeQueryAndPart(AndQuery<DbGamePeriodCB> andCBLambda) {
        xorSQAP((DbGamePeriodCB)this, andCBLambda);
    }
    ...
    protected void xprepareSyncQyCall(ConditionBean mainCB) {
        final DbGamePeriodCB cb;
        if (mainCB != null) {
            cb = (DbGamePeriodCB)mainCB;
        } else {
            cb = new DbGamePeriodCB();
        }
    ...
    protected String xgetConditionBeanClassNameInternally() { return DbGamePeriodCB.class.getName(); }
    protected String xgetConditionQueryClassNameInternally() { return DbGamePeriodCQ.class.getName(); }
